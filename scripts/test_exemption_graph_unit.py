...
from ingest_tuition import (
    _manifest_tuition_sources,
    _tuition_parser_kind,
    parse_exemption_basis,
)
...
def test_unit_exemption_pipeline():
    ...
    # 1b. Test metadata-driven tuition source discovery and dispatch.
    print("\n--- 1b. Kiểm tra discovery theo document_metadata.json ---")
    tuition_sources = _manifest_tuition_sources(ROOT / "data" / "markdown")
    parser_kinds = sorted(_tuition_parser_kind(path, meta) for path, meta in tuition_sources)
    assert parser_kinds == ["clc_tt", "exemption_basis", "k51", "k52", "policy"]
    print(f"  ✓ Phát hiện {len(tuition_sources)} bảng học phí bằng metadata; parser dispatch hợp lệ.")
