...
| Lệnh | Mô Tả |
|:------|:-------|
| `python scripts/build_bm25_index.py` | Tái tạo chỉ mục BM25 từ kho Markdown |
| `python scripts/reindex_all.py build --alias-name ctu_scholarship_docs_current` | Nạp toàn bộ dữ liệu vào Qdrant collection mới |
| `python scripts/reindex_all.py swap --alias-name ... --target-collection ...` | Blue-Green swap Qdrant alias (zero-downtime) |
