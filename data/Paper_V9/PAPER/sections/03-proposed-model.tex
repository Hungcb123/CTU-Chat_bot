...
\begin{itemize}
    ...
    \item \textbf{Domain-specialized agents with asymmetric tools:} Academic, financial, and scholarship agents receive scoped tool lists; the general agent has no direct tool. A prompt gate requests one call only when required parameters are valid. If a clearly tool-backed query omits it, one bounded retry names the required tool; missing parameters instead trigger clarification.
\end{itemize}
