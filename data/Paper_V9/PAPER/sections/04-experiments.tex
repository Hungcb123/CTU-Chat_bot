...
\noindent\textbf{Evaluated Snapshot and Infrastructure:} Reported results use a fixed Gemini 2.5 Flash Lite snapshot through Google Cloud Vertex AI with greedy decoding ($T=0.0$); later deployment-model substitutions are not evaluated. Each query was run $R=3$ times, yielding $2,100$ generation and Ragas turns ($7$ configurations $\times 100$ queries $\times 3$ runs). Neural reranking used an NVIDIA RTX 1060 GPU (4~GB VRAM). Neo4j 5 Enterprise with APOC stored relational curricula; PostgreSQL stored parent chunks (2,800 characters, 100 overlap); and Qdrant indexed child chunks (400 characters, 50 overlap) using a 768-dimensional \texttt{vietnamese-bi-encoder}. The system used \texttt{bge-reranker-v2-m3}, LangGraph StateGraph, and Redis 7.
