...
Evaluations use greedy decoding ($T{=}0.0$) to minimize sampling variance. Paired differences across configurations are computed at the query level ($N{=}100$); 95\% CIs are estimated using 10,000 bootstrap resamples. Three primary pairwise comparisons (Scenario~1) are declared in advance; additional pairwise tests use Holm correction. Scenario~3 degradation is computed as $\Delta = \text{score}_{51} - \text{score}_{11}$ across counterbalanced tool-order blocks.
