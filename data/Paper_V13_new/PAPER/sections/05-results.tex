...
Table~\ref{tab:arch_comparison} compares the three configurations on the 100-query held-out benchmark. Because evidence retrieval is held fixed across configurations to strictly isolate orchestration effects, Source Recall (78.6\%) and Source AP (70.8\%) are identical across all three architectures. Differences in final answer quality and operational cost stem entirely from prompt specialization, tool privilege scoping, and agent routing.
...
(3)~Routed Generic vs.\ Single Agent: comparable token savings ($\Delta{=}-2{,}824$ input tokens) but lower Fact Coverage ($-4.2$\,pp, CI $[-9.8, +1.3]$\,pp), indicating that routing without policy specialization is insufficient.

On the 20 cross-domain queries, CTU-Chat achieves an 85.0\% E2E point estimate compared with 75.0\% (Single) and 70.0\% (Generic), consistent with the intended single-owner multi-evidence-prefetch design. A sensitivity check across fact-coverage thresholds confirms stability: at $\theta{=}0.50$, CTU leads Single 81.0\% vs.\ 73.0\% (+8.0\,pp); at $\theta{=}0.60$, 63.0\% vs.\ 59.0\% (+4.0\,pp); and at strict thresholds ($\theta{=}0.75$: 39.0\% vs.\ 41.0\%; $\theta{=}1.00$: 32.0\% vs.\ 33.0\%), performance remains closely matched, demonstrating that the observed advantage is not an artifact of threshold selection.
...
As the registry scales from 11 to 51 tools, the Full-Registry Single Agent degrades from 86.0\% to 83.3\% E2E while input tokens expand by 124\%. The Global Top-10 drops to 78.7\% (partially from gold tool truncation, as Gold Recall drops from 100.0\% to 96.0\%, and partially from LLM selection error). The Routed Specialist preserves 88.0\% E2E at 51 tools with only 9.8 visible tools. At the 51-tool endpoint, Routed Specialist exhibits a statistically credible advantage over Top-10 Single Agent (+9.3\,pp E2E, 95\% CI $[+2.0, +17.3]$\,pp) and a +4.7\,pp higher point estimate over Full-Registry Single Agent (95\% CI $[-1.3, +11.3]$\,pp, containing zero) while cutting input tokens by 52.4\% (1{,}770 vs.\ 3{,}722). The degradation interaction (Specialist vs.\ Top-10 high-minus-low difference) is $+7.3$\,pp (95\% CI $[0.0, +15.3]$\,pp), confirming an endpoint advantage without demonstrating statistically divergent degradation slopes.
...
CTU-Chat's advantage is concentrated in tool-execution reliability (2 tool errors vs.\ 7; 2 unnecessary calls vs.\ 5). Gold-source absence and reranker-demotion counts are identical (2 and 4 respectively), while CTU-Chat incurs fewer cross-domain partial omissions (3 vs.\ 5), confirming that retrieval quality is an orthogonal bottleneck.
