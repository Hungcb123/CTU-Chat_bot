...
\begin{itemize}
    ...
    \item \textbf{Domain-specialized agents with asymmetric tools:} The academic, financial, and scholarship agents are constructed with separate prompts and tool lists, while the general agent has no direct tool. This confines tool selection to the agent selected by the supervisor and keeps domain-specific evidence paths separate.
\end{itemize}

%------------------------------------------------
\FloatBarrier
\subsection{Domain-Specialized Agents and Asymmetric Privilege Allocation}
\label{sec:agents}

Table~\ref{tab:agent_specs} details the tool bindings and evidence access paths assigned to each specialist. \system{} decouples macro-level domain routing from micro-level action selection: the centralized supervisor dispatches the query to a specific domain, while the assigned specialist operates strictly within its preconfigured tool boundary.

\begin{table}[h!]
\centering
\caption{Operational profiles and direct tool availability of the domain-specialized agents.}
\label{tab:agent_specs}
\resizebox{\columnwidth}{!}{
\begin{tabular}{llll}
\toprule
\textbf{Agent role}  & \textbf{Evidence store} & \textbf{Access path} \\
\midrule
\texttt{Academic}    & Neo4j & ReAct over graph tools \\
\texttt{Financial}   & Neo4j, JSON, document RAG & Retrieval plus calculation \\
\texttt{Scholarship} & Document RAG & Retrieval plus calculation \\
\texttt{General}     & Document RAG & Evidence-grounded synthesis \\
\bottomrule
\end{tabular}
}
\end{table}

\textbf{Direct tools vs. pre-retrieved context:} The academic agent directly selects among six graph tools. The financial agent receives intent-scoped context and may select among three structured lookup tools and \texttt{tinh\_toan\_hoc\_phi}; the scholarship agent receives retrieved policy context and has only \texttt{tinh\_tien\_hoc\_bong}; and the general agent performs evidence-grounded synthesis without direct tools. This asymmetric tool assignment enforces architectural guardrails across agents, restricting tool invocation to valid domains and preventing uncoordinated tool proliferation.
...
The Neo4j graph comprises 10 node labels (7 for curriculum: \texttt{Program}, \texttt{Course}, \texttt{Block}, etc.; and 3 for tuition: \texttt{TuitionFee}, \texttt{TuitionPolicy}, \texttt{ExemptionBasisRate}) connected via 13 relationship types (\texttt{HAS\_BLOCK}, \texttt{REQUIRES}, etc.). Curriculum structure spans seven labels, while tuition and exemption data span three. Narrative regulations remain in the document-retrieval store unless represented by one of these structured entities.

The graph schema models the essential relational dependencies required for academic counseling, capturing program composition, prerequisite chains, learning outcomes, and program-to-tuition links without introducing unnecessary ontological overhead.

The ingestion layer defines thirteen relationship types: \texttt{HAS\_BLOCK}, \texttt{CONTAINS}, \texttt{REQUIRES}, \texttt{PARALLEL\_WITH}, \texttt{HAS\_PLO}, \texttt{HAS\_PEO}, \texttt{REALIZED\_BY}, \texttt{HAS\_TRACK}, \texttt{BELONGS\_TO\_TRACK}, \texttt{HAS\_JOB\_POSITION}, \texttt{HAS\_TUITION}, \texttt{GOVERNED\_BY}, and \texttt{HAS\_EXEMPTION\_BASIS}. They encode program composition, course dependencies, learning outcomes, specialization and career links, and financial-policy associations.
...
\begin{align}
  D &= B\frac{p}{100}, \label{eq:reduction} \\
  P &= \max(0, T-D), \label{eq:payable}
\end{align}
...
\textbf{Advantages of the Algorithmic Separation:}
\begin{itemize}
    \item \emph{External tool augmentation:} The LLM handles natural-language interpretation, while a deterministic component performs validation and arithmetic. This extends the system's calculation capability without relying on unconstrained numerical generation by the LLM.
    \item \emph{Distinct monetary bases:} The calculation keeps actual tuition $T$ separate from exemption basis $B$, applies the percentage only to $B$, validates its range, and prevents a negative payable amount through explicit clamping.
\end{itemize}
