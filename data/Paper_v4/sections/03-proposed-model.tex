...
\begin{itemize}
    ...
    \item \textbf{Two-Tier Conditional Routing Edges:} Instead of letting agents dynamically negotiate execution flow, orchestration is governed by deterministic conditional dispatch: $\mathrm{Edge}_{\text{sup}}(\mathcal{S}) = \text{'academic\_agent'}$ if $\mathcal{S}[\text{'next\_agent'}] = \text{'academic'}$, and $\text{'retrieval'}$ otherwise, directly bypassing document retrieval for curriculum queries. At the retrieval exit, $\mathrm{Edge}_{\text{ret}}(\mathcal{S})$ dispatches retrieved context to the targeted financial, scholarship, or general agent.
...
$\mathcal{S}[\texttt{'search\_query'}] \leftarrow q_{\text{standalone}}$;\quad $\mathcal{S}[\texttt{'next\_agent'}] \leftarrow d^*$\;
$v_{\text{next}} \leftarrow \mathrm{Edge}_{\text{sup}}(\mathcal{S})$ \tcp*{Conditional dispatch (Section~\ref{sec:architecture})}
...
For fee-reduction requests, \system{} augments the LLM with the external deterministic tool \texttt{tinh\_toan\_hoc\_phi}. The LLM interprets the request and supplies three parameters---the actual per-credit tuition $T$, the per-credit exemption basis $B$, and the percentage entitlement $p$---but does not perform the arithmetic through free-form text generation. The external tool validates the percentage range and applies the fixed calculation in Eq.~\eqref{eq:reduction}:
\begin{equation}
  D = B \cdot (p / 100), \qquad P = \max(0, T - D), \label{eq:reduction}
\end{equation}
where $D$ is the reduction per credit and $P$ is the remaining payable amount per credit (in VND). The tool enforces $0 \le p \le 100$, formats outputs as integer amounts, and clamps $P \ge 0$. This algorithmic separation delegates numerical evaluation to deterministic code, keeping actual tuition $T$ distinct from exemption basis $B$ and eliminating arithmetic hallucination from the generative model.
...
\midrule
Program/course attributes      & Neo4j property lookup  & Exact or normalized key match \\
Prerequisites/shared courses   & Neo4j traversal        & Stored graph topology \\
Regulation identifiers         & BM25 child index       & Exact decree token match \\
Paraphrased regulations        & Hybrid RRF + reranker  & Lexical + semantic signals \\
Fee/scholarship calculation    & Python tool            & Deterministic formula outside LLM \\
\bottomrule
\end{tabular}
}
\end{table}
