...
\textbf{Primary Endpoint Definitions.} In Scenarios~1 and 2, the primary decision endpoint is the \emph{Fact-Grounded Pass Rate} ($\text{Pass}_{\ge 50\%}$), defined programmatically as the proportion of answers achieving at least 50\% Required-Fact Coverage ($\text{FactCov} \ge 0.50$), complemented by continuous Fact Coverage. In Scenario~3, the primary endpoint is \emph{Tool E2E Success}, requiring correct tool selection, valid arguments, and error-free execution under expanding candidate registries. For adversarial queries, success requires absence of unnecessary tool calls. For the arithmetic subset ($N{=}9$), numeric exact match against bursar schedules is independently audited.
