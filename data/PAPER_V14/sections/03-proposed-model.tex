...
\system{} specializes LangGraph through typed state (\texttt{AgentState} with validated rewrite, routes, evidence, and response), an intent--owner contract that checks structured supervisor output before dispatch, conditional routing (academic queries go directly to the graph agent; others through retrieval), acyclic orchestration (specialists return to \texttt{END} without peer edges), and asymmetric tool registries (each specialist sees only its scoped tools, reducing per-query token cost and limiting the semantic-ambiguity surface as the tool registry scales).

%------------------------------------------------
\subsection{Neo4j Knowledge Graph and Cypher Tools}
\label{sec:neo4j_graph}

The Neo4j graph is built from 114 Markdown source files covering 111 undergraduate curricula and fee schedules. The resulting graph stores approximately 7{,}000 nodes across 10 labels---curriculum entities (\texttt{Program}, \texttt{Course}, \texttt{Block}, \texttt{PLO}, \texttt{PEO}, \texttt{SpecializationTrack}, \texttt{JobPosition}) and tuition entities (\texttt{TuitionFee}, \texttt{TuitionPolicy}, \texttt{ExemptionBasisRate})---connected by over 16{,}000 edges across 13 typed relationships encoding containment, prerequisites, program outcomes, tracks, occupations, tuition, and exemption bases.
