...
(1)~CTU-Chat vs.\ Single Agent: CTU-Chat achieves a 10.0\,pp higher Automated Fact Pass Rate ($\text{Pass}_{\ge 50\%}$) point estimate (76.0\% vs.\ 66.0\%, paired 95\%\,bootstrap CI $[+2.0, +19.0]$\,pp; exact McNemar $p{=}0.0414$), alongside higher Fact Coverage ($\Delta{=}+3.58$\,pp, 95\%\,CI $[-1.2, +8.5]$\,pp) and 41.4\% fewer specialist input tokens (26.1\% fewer total specialist tokens: $\Delta{=}-2{,}044$, CI $[-2{,}710, -1{,}378]$; Single: 7{,}826 vs.\ CTU: 5{,}782; supervisor adds ${\sim}650$ input tokens/query), under differing measurement boundaries (10{,}652\,ms end-to-end multi-agent pipeline vs.\ 5{,}159\,ms post-retrieval single-agent generation).
...
\textbf{Latency and Token Considerations:} Across configurations, latency reflects distinct operational boundaries: Single Agent ($5{,}159$\,ms) measures inference post-retrieval, whereas multi-agent variants ($8{,}921$\,ms for Routed Generic, and $10{,}652$\,ms for \system{}) measure pipeline execution inclusive of routing and contract dispatch. Within multi-agent execution, the $+1{,}731$\,ms delta between Routed Generic and \system{} corresponds to specialist prompt instructions producing $+677$ additional output tokens ($2{,}092$ vs.\ $1{,}415$) for domain-tailored reasoning and evidence formatting. In ablation S2-A4, the system showed $4{,}540$\,ms lower latency when deterministic calculators were disabled. Regarding token usage, specialist prompt scoping reduces recorded agent input tokens ($3{,}690$ vs.\ $6{,}296$ in Table~\ref{tab:arch_comparison}), while supervisor routing introduces an unlogged upstream call averaging ${\sim}650$ input tokens per query.
...
Table~\ref{tab:failures} categorizes error modes across the 58 failure instances ($<50\%$ Fact Coverage) in Scenario~1 (34 for Single Agent, 24 for CTU-Chat).
