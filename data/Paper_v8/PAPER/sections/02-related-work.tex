...
Unlike whitespace-delimited languages, Vietnamese is an isolating, tonal language where white spaces separate syllables rather than words~\cite{nguyen2020phobert}, with over 85\% of lexical units forming multi-syllabic compound words (e.g., \textit{``h\d{o}c ph\'i''} [tuition fee]). While domain-adapted bi-encoders like PhoBERT capture semantic paraphrasing, Vietnamese higher education decrees rely heavily on rigid Sino-Vietnamese legal terminology and alphanumeric cohort codes. This dual characteristic motivates coupling syllable-aware dense embeddings with exact lexical token matching to mitigate both semantic drift and alphanumeric recall failures.
...
Crucially, this analysis exposes three key operational gaps in automated university counseling: (1)~\emph{Lack of Bounded Multi-Agent Orchestration}: monolithic pipelines and unconstrained agent meshes lack execution guardrails, frequently triggering tool mis-selection and parameter hallucination; (2)~\emph{The Uniform Representation Bottleneck}: forcing relational curricula, tabular tuition matrices, and narrative legal text into a single retrieval space can cause prerequisite fragmentation and semantic drift; and (3)~\emph{Absence of Representation-Aware Routing}: dynamic steering mechanisms that allocate queries across isolated representation pathways while offloading statutory arithmetic to deterministic engines remain largely unaddressed in higher-education administrative counseling. These structural imperatives directly motivate the centralized supervisor-routed architecture detailed in Section~3.
