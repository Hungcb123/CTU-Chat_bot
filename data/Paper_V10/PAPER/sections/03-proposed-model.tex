...
\begin{itemize}
    ...
    \item \textbf{Asymmetric tools and deterministic validation:} Academic, financial, and scholarship agents receive scoped tool registries, while the general agent has none. Tool invocations pass through an argument validator before execution; missing mandatory parameters or out-of-range values intercept invocation and trigger user clarification. In addition, one bounded retry prompts the specialist when an expected tool call is omitted.
\end{itemize}
...
For narrative regulations and scholarship criteria, evidence is acquired via parent-resolved retrieval: BM25 and Qdrant search 400-character child chunks, their parent sections are loaded from PostgreSQL, and rankings are fused via $s_{\mathrm{RRF}}(d)=\sum_{s\in\{\mathrm{bm25},\mathrm{dense}\}}(60+r_s(d))^{-1}$. In production runtime, retrieval executes over intent-governed metadata lanes to minimize latency. In the evaluated full experimental configuration (evaluated in Section~4 as E5/T4), candidate pools are augmented with the top 20 unfiltered hybrid candidates and top BM25 anchors, deduplicated, and reranked using \texttt{BAAI/bge-reranker-v2-m3}. Scores below 0.20 are pruned unless fewer than two candidates remain; graph evidence is subsequently incorporated under fixed quotas, and up to seven passages are packed for generation.
