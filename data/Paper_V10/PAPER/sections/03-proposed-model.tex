...
\begin{itemize}
    ...
    \item \textbf{Node-scoped state updates:} \texttt{AgentState} retains the validated rewrite, raw and repaired routes, repair reason, retrieved evidence, and response. The supervisor writes routing fields, retrieval writes \texttt{context} and \texttt{retrieval\_instruction}, and the specialist writes \texttt{response}, making route repair traceable.
    ...
    \item \textbf{Domain-specialized agents with asymmetric tools:} Academic, financial, and scholarship agents receive scoped tool lists; the general agent has no direct tool. A prompt gate requests one call only when required parameters are valid. If a tool-backed query omits an invocation, one bounded retry names the required tool; missing parameters instead trigger clarification.
\end{itemize}
...
\label{alg:routing}
\KwIn{Student query $q$, Dialogue history $\mathcal{H}$, Shared state $\mathcal{S}$, Ownership contract $\mathcal{P}$}
...
$\langle d_{\mathrm{raw}}, \iota_{\mathrm{raw}} \rangle \leftarrow \mathrm{StructuredSupervisor}(q)$\;
\If{structured classification fails}{
    $d_{\mathrm{raw}} \leftarrow \text{'general'}$; $\iota_{\mathrm{raw}} \leftarrow \text{'other'}$\;
}
$\langle d^*, \iota^*, \rho \rangle \leftarrow \mathrm{RepairRoute}(q,d_{\mathrm{raw}},\iota_{\mathrm{raw}},\mathcal{P})$\;
Write $q_{\text{standalone}},d_{\mathrm{raw}},\iota_{\mathrm{raw}},d^*,\iota^*,\rho$ to $\mathcal{S}$\;
\eIf{$d^* = \text{'academic'}$}{
    $v_{\text{next}} \leftarrow \text{'academic\_agent'}$\;
}{
    \If{$d^* = \text{'financial'}$}{
        Prefetch tuition evidence from Neo4j, else JSON\;
    }
...
Rewrites require validation, and structured classification falls back to \texttt{general}/\texttt{other}. \texttt{RepairRoute} preserves valid predictions but corrects strong rule conflicts or incompatible agent--intent pairs under $\mathcal{P}$. Conditional edges send academic queries to the graph agent and other domains through retrieval.
...
\begin{enumerate}
    ...
    \texttt{BAAI/bge-reranker-v2-m3}. In the evaluated full configuration,
    candidates from intent-governed lanes are pooled with the top 20 candidates
    from unfiltered hybrid retrieval and the top-ranked BM25 candidates before
    deduplication. Candidates with reranker scores below 0.20 are removed unless
    fewer than two remain. Graph evidence is then added under predefined quotas,
    and up to seven passages are packed for generation.
\end{enumerate}
