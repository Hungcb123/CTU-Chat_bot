...
\emph{Objective \& RQ Alignment:} Scenario~2 primarily addresses \textbf{RQ2} by benchmarking the proposed full system (T4) against three single-agent RAG baselines (T1--T3) and three modular ablations (T5--T7) on the Held-Out Test Set ($1,050$ total evaluation turns; Table~\ref{tab:e2e_evaluation}). Its T4--T7 comparison additionally provides configuration-level evidence for the answer-accuracy component of \textbf{RQ3}.
\emph{Results \& Interpretation:} As reported in Table~\ref{tab:e2e_evaluation}, T4 attains the highest Fact EM among the evaluated configurations ($62.03\%$ vs. $55.72\%$ for BM25, a difference of 6.31 percentage points), indicating stronger performance on the evaluated numerical tuition and credit cases. Relative to BM25, T4 also records higher Context Precision ($0.6977$ vs. $0.6674$), Context Recall ($0.8933$ vs. $0.8800$), Source AP ($0.9311$ vs. $0.8954$), and Answer Relevancy ($0.6195$ vs. $0.6151$), whereas BM25 records higher Answer Correctness ($0.6735$ vs. $0.6637$). The complementary contributions of modular cross-reranking (T5), graph grounding (T6), and subspace governance (T7) are analyzed in detail under Discussion (Section~\ref{sec:discussion}).
...
Financial \& Scholarship Tools & 5 functions & 92.0\% & 88.0\% & 90.7\%  & 86.7\% & 98.7\% & 66.2\% \\
...
\textbf{Answering RQ1 (Domain-Specialized Multi-Agent Architecture):} Centralizing routing under a structured supervisor with asymmetric privileges and bounds ($\text{max\_tool\_calls}=1$, timeout $=60\,\text{s}$) maintained bounded decisions: the supervisor achieves $97.0\%$ agent accuracy, $97.8\%$ Macro-F1, and $95.0\%$ intent accuracy across 300 runs, outperforming rule-based routing ($85.0\%$, $69.2\%$). Specialist tools reached $100.0\%$ end-to-end pass on 8 of 12 individual functions (averaging $98.9\%$ on Academic and $86.7\%$ on Financial/Scholarship tools), while adversarial testing (Panel~C) verified $100.0\%$ tool-gating accuracy, $98.3\%$ safe-result behavior, and $100.0\%$ bounded completion. A paired scalability test further compared the multi-agent architecture against a monolithic single-agent baseline: with only 11 production tools, the single agent achieved comparable E2E pass ($93.3\%$ vs. $88.3\%$); however, when 16 synthetic distractor tools were introduced to stress-test decision-space scaling, multi-agent tool selection remained at $100.0\%$ while the monolithic agent degraded from $98.3\%$ to $91.7\%$ (paired $\Delta=+8.3\%$, 95\% bootstrap CI $[+1.7, +15.0]$, $n=60$), confirming that domain partitioning mitigates decision-space pollution under tool-space expansion.
...
\textbf{Answering RQ3 (Representation-Aware Retrieval Effectiveness and Answer Accuracy):} The configuration sequence reveals that neural reranking principally improves early-rank ordering rather than evidence coverage: E3-to-E4 gains are concentrated in Hit@1 and MRR@10, while the full stack does not surpass the lexical baseline on P@5 or Recall@5. Domain stratification further shows that E5's additional Hit@1 benefit is concentrated in Financial queries; General queries improve over E1 by E4 and then remain unchanged, while neither Academic nor Scholarship improves over E1. This pattern is consistent with BM25 remaining competitive on exact course codes, cohort identifiers, and administrative terminology. For answer accuracy, T4 is numerically higher than the ungoverned T7 configuration in AC ($0.6637$ vs. $0.6430$), but the paired difference ($0.0207$) has a 95\% confidence interval spanning zero ($[-0.0062, 0.0490]$) and therefore does not establish a conclusive improvement. Because E5 and T4--T7 change evidence-pool construction together with governance, these results provide configuration-level comparisons and do not attribute the observed differences causally to routing alone.
