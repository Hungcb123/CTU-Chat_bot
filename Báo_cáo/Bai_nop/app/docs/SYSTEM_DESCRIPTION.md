# Chatbot Học bổng ĐHCT — Mô tả Hệ thống
...
3. Re-rank: `ContextualCompressionRetriever` bọc `CrossEncoderReranker(top_n=3)` — 10 parent vào, giữ 3 tốt nhất.
...
- **Tính năng:** modal đăng nhập/đăng ký, kiểm tra trạng thái auth khi tải, gửi chat với hiệu ứng đang gõ (không streaming thật), nút New Chat / Clear History, render markdown câu trả lời.
...
4. **BGE reranker-v2-m3** chấm lại 10 parent, giữ top 3 → ghép thành chuỗi `{context}`.
...
- **Secret trong `.env`:** `GOOGLE_API_KEY`, `GROQ_API_KEY`, `LLAMA_CLOUD_API_KEY`... đang ở dạng plaintext. Nên xoay vòng key và đưa ra khỏi version control.
